...
export class Ym2612 {
  ...
  static async create(options = {}) {
    ...
    const api = {
      ...
      generateWithInternalEnvelope: optionalCwrap(
        module,
  ...
  read(offset) {
    if (typeof this.api.read !== "function") {
      throw new Error("This runtime does not support read(offset).");
    }
    return this.api.read(this.handle, offset);
  }

  readStatus() {
    if (typeof this.api.readStatus === "function") {
      return this.api.readStatus(this.handle);
    }
    return this.read(0);
  }
  ...
  generateStereoWithInternalEnvelope(frames, channel = 0) {
    if (typeof this.api.generateWithInternalEnvelope !== "function") {
      const { left, right } = this.generateStereo(frames);
      return {
        left,
        right,
        envelopes: [
